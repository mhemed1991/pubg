...
public:
	...
	void JMP(uint32_t FakeReturnAddress, uintptr_t addr)
	{
		DWORD OldProtect1;
		uint8_t hookbytes[8] = { 0x04, 0xF0, 0x1F, 0xE5, 0xDE, 0xAD, 0xBE, 0xFF };//JMP ShellCode
		*(uint32_t*)(hookbytes + 4) = FakeReturnAddress;//JMP Address
		uint32_t OLDProtect = ChangeProtection(addr, sizeof(hookbytes), PAGE_READWRITE, OldProtect1);//ChangeProtect
		WriteMemory(addr, (uintptr_t)&hookbytes, sizeof(hookbytes));//WriteMemory
		ChangeProtection(addr, sizeof(hookbytes), PAGE_READWRITE, OldProtect1);//BackOldProtect
	}
